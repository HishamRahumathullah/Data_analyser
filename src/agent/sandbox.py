    ...
    def __init__(self, config: Optional[SandboxConfig] = None):
        ...
        if self._use_docker:
            try:
                ...
                logger.warning(
                    f"Docker not available: {e}, falling back to subprocess sandbox"
                )
...
"""
            ...
            try:
                ...
                logger.error(
                    f"Sandbox execution timed out after {self.config.timeout}s"
                )
                return {
                    "success": False,
                    "error": f"timeout: execution timed out after {self.config.timeout} seconds",
                    "figure": None,
                }
            except Exception as e:
