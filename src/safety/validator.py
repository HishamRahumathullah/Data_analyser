...
class SQLSafetyValidator:
    ...
    ALLOWED_COMMANDS = {"SELECT", "WITH", "EXPLAIN", "DESCRIBE", "SHOW", "PRAGMA"}
    ...
    def __init__(
        self, allowed_tables: Optional[Set[str]] = None, read_only: bool = True
    ):
    ...
    def validate(self, query: str) -> Dict:
        ...
        try:
            ...
            for stmt in parsed:
                ...
                if cmd_type not in self.ALLOWED_COMMANDS:
                    if cmd_type in self.FORBIDDEN_COMMANDS:
                        ...
                        result["errors"].append(
                            f"Unknown/unsupported operation: {cmd_type}"
                        )
                ...
                for sq_table in subquery_tables:
                    if sq_table.lower() in self.FORBIDDEN_TABLES:
                        result["errors"].append(
                            f"Forbidden table in subquery: {sq_table}"
                        )
                    elif (
                        self.allowed_tables
                        and sq_table.lower() not in self.allowed_tables
                    ):
                        result["errors"].append(
                            f"Unauthorized table in subquery: {sq_table}"
                        )
            ...
            log_security_event(
                "sql_parse_failure", {"query": query[:100], "error": str(e)}
            )
            return result
        except Exception as e:
            result["errors"].append(f"Validation error: {str(e)}")
            logger.error(
                f"Unexpected validation error: {e}", extra={"query": query[:100]}
            )
            return result
